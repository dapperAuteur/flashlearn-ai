...
## FlashLearnAI.WitUS.Online Adds Ecosystem API for Schools and Districts — COPPA Cascade-Delete and Signed Webhooks for LMS / SIS Integration
...
**[City, State] — April 2026** — Following the March 2026 launch of the Public API for Schools, FlashLearnAI.WitUS.Online today announced the **Ecosystem API** — an integration layer that lets schools, districts, and edtech vendors plug FlashLearn into their existing LMS or Student Information System (SIS) without exposing student PII to FlashLearn, complete with documented COPPA cascade-delete and HMAC-signed outbound webhooks for SIS callbacks.
...
> [QUOTE: 1–2 sentence statement from BAM as founder — e.g., what changed for K-12 procurement between March's Public API launch and today's Ecosystem API.]

### What's new since March

The original Public API for Schools (announced March 2026) is now joined by:

- **4 new endpoints** for child-scoped session scheduling, mastery rollups, and COPPA cascade-delete (now **27 total** in the Public API).
...
- **Per-endpoint secret encryption** — AES-256-GCM at rest. Plaintext shown once at registration; rotation self-service.
...
The Ecosystem API ships all three as documented, tested primitives. The COPPA cascade-delete is covered by an end-to-end test in CI; deleting a student purges sessions, attempts, mastery rollups, and decks across five collections, then writes an audit-log row.

### District use cases

**LMS integration (Canvas, Schoology, Google Classroom, etc.):**
- Teacher creates a unit in the LMS; LMS calls `POST /api/v1/sessions` to schedule a 5-card review deck for each enrolled student the morning after the unit completes
...
- Student withdrawal (transfer, graduation) triggers `DELETE /api/v1/children/:studentId` for verifiable purge — receive an audit receipt for the student's permanent record
...
- Indiana Kindergarten standards seeded in production today; additional state frameworks supported via a JSON upload (contact us for your state)

### Why "Ecosystem" not "Public" for districts

The Public API (`fl_pub_*` keys) is self-serve — anyone can sign up and generate. The Ecosystem API (`fl_eco_*` keys) is admin-issued because it touches child-scoped data and dispatches outbound webhooks. We vet integrations against COPPA / data-handling expectations before issuing a key. For most districts and edtech vendors integrating into an SIS or district LMS, the ecosystem path is the correct one.
...
3. For LMS / SIS integration with COPPA cascade-delete, email [admin.flashlearnai@awews.com](mailto:admin.flashlearnai@awews.com) with your district name and integration plan to request an `fl_eco_` key.
