...
// E2E + a11y gate (witus plan 30 Phase 2, ported from witus feat/30-playwright-ci). Two ways to
...
// No webServer block on purpose: this repo's dev server needs real env (MONGODB_URI, NextAuth
// secrets, AI provider keys) that CI and fresh clones don't have. CI always runs against a
...
export default defineConfig({
  ...
  projects: [
    ...
    {
      // Mobile-first is a charter rule ecosystem-wide; a flow that only works on desktop is a
      // failing flow. Same specs, small viewport.
      name: "mobile",
      use: {
        ...devices["Pixel 7"],
