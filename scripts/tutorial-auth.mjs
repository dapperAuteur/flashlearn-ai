...
const PROFILES = {
  user: {
    who: "bam@awews.com — the ordinary learner account (deck-flow spends one AI generation per run)",
    url: "https://flashlearnai.witus.online",
    file: ".auth/tutorial-user.json",
    // 401 signed out, 200 signed in. Page routes here render for everyone and gate their contents
    // client-side, so they prove nothing.
    gated: "/api/user/profile",
    signInPath: "/auth/signin",
  },
  teacher: {
    who: "bam+demo@awews.com — the DEMO teacher, never your own account (yours teaches real classrooms)",
    url: "https://flashlearnai.witus.online",
    file: ".auth/tutorial-teacher.json",
    gated: "/api/classrooms",
    signInPath: "/auth/signin",
    // Proves both halves: a teacher session AND the one that owns the demo classroom.
    expectBody: "6ab17c2c98befe23d2efcdc8",
    expectBodyLabel: "the Tutorial Demo Classroom",
    note: "If /teacher is empty after signing in, sign out and in again: the Teacher role is carried in the session.",
  },
};
