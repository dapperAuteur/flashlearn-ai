...
// Tutorial-as-test harness (plan 30 §8.3). COPIED VERBATIM FROM THE ECOSYSTEM REFERENCE at
// gemini/witus/e2e/tutorials/tutorial.ts — edit it there and re-copy, rather than diverging here;
// compose.mjs in this repo is the matching copy of the same reference.
//
// A tutorial is an ordered list of steps; each step has a
// `title` (becomes the doc heading), a `narration` (the line BAM records — see
// witus plans/31-tutorial-narration-scripts.md), and an `action` that drives the page. Running a tutorial
...
// Coverage of this repo's help articles is tracked in e2e/tutorials/manifest.json; keep it in step
